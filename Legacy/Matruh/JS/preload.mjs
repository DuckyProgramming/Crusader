...
export function preload(){
    ...
    types.map.forEach(map=>{
        graphics.load.map.push(map.term.map(term=>loadImage(`${root}Assets/Matruh/map/${term}.png`)))
    ...
    graphics.load.water=loadBytes(`${root}Assets/Matruh/data/water.bin`)
    graphics.load.fortifications=loadBytes(`${root}Assets/Matruh/data/fortifications.bin`)
}
