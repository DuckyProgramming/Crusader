...
export function preload(){
    ...
    types.map.forEach(map=>{
        graphics.load.map.push(map.term.map(term=>loadImage(`${root}Assets/Gazala/map/${term}.png`)))
    ...
    graphics.load.water=loadBytes(`${root}Assets/Gazala/data/water.bin`)
    graphics.load.fortifications=loadBytes(`${root}Assets/Gazala/data/fortifications.bin`)
}
