...
async function main(){
    const pixels=await loadImagePixels(`../map/fortifications.png`)
    const bits=[]
    let lx=pixels[1][0]
    let ly=pixels[1][1]
    let set=[
    ...
    for(let a=0,la=24;a<la;a++){
        set.push([Math.round(Math.sin(a/la*360)*15),Math.round(Math.cos(a/la*360)*15)])
    }
    ...
    fs.writeFileSync(`fortifications.bin`,buffer);
    console.log(`Wrote ${byteArray.length} bytes to fortifications.bin`);
}
